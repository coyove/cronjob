...
func IfString(v bool, a, b string) string {
	if v {
		return a
	}
	return b
...
func HashPassword(password string) string {
	pwdHash := hmac.New(sha256.New, Cfg.KeyBytes)
	pwdHash.Write([]byte(password))
	return base64.URLEncoding.EncodeToString(pwdHash.Sum(nil)[:12])
}

func UUID(size int) string {
	buf := make([]byte, size*2)
	rand.Read(buf[:size])
	base64.URLEncoding.Encode(buf, buf[:size])
	return string(buf[:size])
}

func WatchTime(s time.Time, n string, a ...interface{}) {
	if e := time.Since(s).Seconds(); e > 0.5 {
		log.Println("WatchTime:", n, " too slow (", e, ")", a)
	}
}
