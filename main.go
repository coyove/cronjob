...
import (
	...
	"github.com/coyove/cronjob/handler"
...
func main() {
	...
	arangoConn, err := ahttp.NewConnection(ahttp.ConnectionConfig{
	...
	dal.Init("cj", redisConfig, driver.ClientConfig{
		Connection: arangoConn,
		// Authentication: driver.BasicAuthentication("root", ""),
	})

	// log.Println(dal.SendPrivateMessage(&model.UserInboxMessage{
	// 	From: "test",
	// 	To:   "zzzz",
	// }))
	// log.Println(dal.SendPrivateMessage(&model.UserInboxMessage{
	...
	// 	To:   "zzzz",
	// }))
	// mmm := &model.UserInboxMessage{
	// 	From: "test22",
	// 	To:   "zzzz",
	// }
	// log.Println(dal.SendPrivateMessage(mmm))
	// log.Println(mmm.ID)
	// log.Println(dal.DismissUserInbox("zzzz", "test22"))
	// log.Println(dal.ListUserInboxOrSent("inbox", "zzzz", "", 0, 0, "", true, 0, 4))
	...
	r.SetFuncMap(template.FuncMap{
		"session": func() string {
			return "s" + strconv.FormatInt(time.Now().Unix()<<32|int64(rand.Uint32()), 36)
		},
	...
	r.Handle("POST", "/api2/login_records", handler.APILoginRecords)
